...
//QUANTIFICATION CODE			
		...
		//1: SETUP AND ERROR HANDLING
					...
					if(!auto & cmd != "Single Channel Quantification"){
						//GUI for perfusions
						Dialog.create("Is your tissue perfused?");
						Dialog.addChoice("Perfusion", newArray("yes", "no"), "yes");

						Dialog.show();

						perf = Dialog.getChoice();


						//BLOOD VESSEL CORRECTIONS
						if(perf == "yes"){
							toRun = newArray(false, false, false);
							run("Close All");
						}
						if(perf == "no"){
							//GUI for selecting corrections to run
							Dialog.create("Autofluorescence");
							Dialog.addMessage("Run quantification for tissue that has not been perfused.");
							Dialog.addMessage("Ensure you have your MOST autofluorescent image open (try the 550 channel).");

							vals = newArray("Background Averaging", "HyD Correction", "Blood Vessel Filtering");
							defs = newArray(true, true, true);
							Dialog.addCheckboxGroup(3, 1, vals, defs);

							Dialog.show();

							//Get User Input
							toRun = newArray(vals.length);
							for (i = 0; i < vals.length; i++) {
								toRun[i] = Dialog.getCheckbox();
							}
						

							/////////RUN IMAGE CORRECTIONS AND SAVE
							//HYD CORRECTION - REMOVE OUTLIERS
							if (toRun[0]) {
							...
							//Normal Quantification BG correction
							run("Subtract Background...", "rolling=50");


							////////BLOOD VESSEL FILTERING
							if(toRun[2]){
								//Ask user if auto or manual BV filtering
								Dialog.create("Blood Vessel Thresholding");
								Dialog.addChoice("Thresholding Type:", newArray("Automatic", "Manual"), "Automatic");
								Dialog.show();

								bv = Dialog.getChoice();
								//convert to mask with wide
								...
								//PREVIEW INPUT WITH A GUI HERE FOR CIRC + SIZE
								//GUI HERE
	
								//create the ROIs
								run("Analyze Particles...", "size=25-Infinity circularity=0.00-0.75 display clear add"); 

	
								//create mask for removing blood vessels
								newImage("bloodvessel_mask", "8-bit white", w, h, 1);
								...
								for (i = 0; i < n; i++) {
   									roiManager('select', i);
    								// process roi here
   								 	Roi.setFillColor("black")
								}
	
								//create new, flattened image
								roiManager("Show All without labels");
								run("Flatten");

								//SAVE BV MASK IN A NEW FOLDER

								//close unnecessary windows
								selectWindow("bloodvessel_mask");
								close();
							
								//save mask for reopenning + close down all files
								selectWindow("bloodvessel_mask-1");
								stackFile = corDir + "bloodvessel_mask.tif";
								saveAs("Tiff", stackFile);
								close();
								run("Close All");
							}
				...
				/////IMAGE CORRECTIONS
							...
							if(opt==cc[1]){
								...
								if(perf=="yes"){
									...
									quant(name, dir, 1, false, "Triangle");
								}
							}
							else{
								//perfused manual
								if(perf=="yes"){
									if(toRun[2]){
										quant(toquant, dir, 1, false, "MaxEntropy");
									}
									quant(name, dir, 0, false, "MaxEntropy");
								}
								else{
									quant(name, dir, 0, false, "Triangle");
								}
							}
							...
							if(toRun[2]){
								curImage = getTitle();
								autoOpen(corDir, "bloodvessel");
								//multiply images to remove autofluorescence
								imageCalculator("Multiply create", "bloodvessel_mask.tif", curImage);

								curImage = getTitle();
								toquant = "Result of bloodvessel_mask.tif";
								selectWindow(toquant);
								close("\\Others");
								rename(name);
								//print("the value of 'name' is " + name);
								deb = getTitle();
								//print("current image to be saved is called" + deb);
								path = ovDir+deb;
								//print("tosave " + path); 
								saveAs("Tiff", path);

							}
							if((perf=="no") & (toRun[2]==false)){
								deb = getTitle();
								//print("current image to be saved is called" + deb);
								path = ovDir+deb;
								//print("Check point #1 " + path); 
								saveAs("Tiff", path);
								
								//fix weird naming bug here for perfused, non bv filtered R1
								if(i==1){
									name = substring(name, 0, lengthOf(name)-6);
									//print(name);
									name = name + ".tif";
								}
