...
function makeWindow(img, name, z){
	...
	//reviewer comment - what happens when image is a perfect square?
	if(w==h){
		side = h;
	}
...
//STEP 4: NONLINEAR REGISTRATION:
	...
	//(Code augemented by kaitlin sullivan for brevity and less user checks
			...
			//4C: REGISTRATION LOOP
				...
				//reviewer comment - include a prompt to continue to NL reg
				Dialog.create("Nonlinear Registration");
				Dialog.addMessage("Would you like to continue with nonlinear registration?");
				Dialog.addChoice("", newArray("Yes", "No"));
				Dialog.show();
				
				nlreg = Dialog.getChoice();
